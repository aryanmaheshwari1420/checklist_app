              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    // Note: TemplateModel keeps categories/items keyed by
                    // plain name (its own simple structure, separate from
                    // the checklist's ID-based categories) — that's
                    // intentional and unrelated to this refactor.
                    ListView.builder(
                      shrinkWrap: true,
            ...
            child: Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () {
                    // loadFromTemplate() sets the FULL draft state — title,
                    // description, type, and categories/items with freshly
                    // generated categoryIds. This is the single source of
                    // truth for the create-from-template flow.
                    ref
                        .read(checklistControllerProvider.notifier)
                        .loadFromTemplate(template);

                    // tempChecklist is passed ONLY so CreateCheckListScreen
                    // can prefill its title/description/dueDate text fields
                    // in its initState. It intentionally carries NO
                    // categories/items — those already live in the draft
                    // state set above via loadFromTemplate(), and must NOT
                    // be touched again.
                    //
                    // IMPORTANT: CreateCheckListScreen's initState currently
                    // calls checklistControllerProvider.notifier.loadChecklist()
                    // when widget.checklist != null, which would overwrite
                    // the categories/items we just loaded (loadChecklist()
                    // always resets items to {}). That call must be skipped
                    // for this flow — see create_checklist_screen.dart fix.
